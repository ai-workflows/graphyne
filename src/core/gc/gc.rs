...
impl<T> GarbageCollector<T> {
    ...
    pub fn allocate(&mut self, data: T) -> usize where T: GarbageCollectable<T> {
...
impl<T> GCPointer<T> where T: GarbageCollectable<T> {
    ...
    pub fn new(value: T, gc: Arc<Mutex<GarbageCollector<T>>>) -> Self where T: GarbageCollectable<T> {
