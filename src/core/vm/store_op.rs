...
pub enum StoreOp<'a>  {
    ...
    StoreFunctionVal(Vec<&'a ValueReference<'a>>, Option<&'a ValueReference<'a>>),
...
impl<'a> StoreOp<'a> {
    ...
    pub fn get_stored_data(self) -> Option<StoredData> {
        match self {
            ...
            StoreOp::StoreFunction(input_vals, output_vals, constants) => Some(StoredData::FuncStored(FuncSig{input_vals: input_vals.iter().map(|v| v.pointer.clone()).collect(), output_vals: output_vals.iter().map(|v| v.pointer.clone()).collect(), constant_vals: constants.iter().map(|v| v.pointer.clone()).collect()})),
            StoreOp::StoreFunctionVal(dependents, constant) => Some(StoredData::FuncValStored(FuncVal{guid: uuid::Uuid::new_v4().to_string(), dependents: dependents.iter().map(|v| v.pointer.clone()).collect(), constant: constant.map(|v| v.pointer.clone())})),
            StoreOp::StoreFunctionOp(op_code, input_vals, output_val) => Some(StoredData::FuncOpStored(FuncOp{opcode: op_code, input_vals: input_vals.iter().map(|v| v.pointer.clone()).collect(), output_val: output_val.pointer.clone()})),
            _ => None,
        }
    }
}