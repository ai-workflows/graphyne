...
impl VM {
    pub fn execute_init(&self, obj_type_ref: &ValueReference, args: Vec<&ValueReference>) -> ExecResult<Vec<ValueReference>> {
        let obj_type = match self.get_ref_value(obj_type_ref)? {
        ...
        for (i, field) in obj_type.2.iter().enumerate() {
            ...
            let expected_type: TypeLive = match self.get_ptr_value(expected_type_ptr) {
            ...
            let arg = &args[i];
            let arg_value: StoredData = self.get_ref_value(arg)?;

            // do a type check if it isn't dynamic
            match expected_type {
                TypeLive::Dynamic => (),
                _ => {
                    let arg_type_ptr = match arg_value.as_live().type_of(&self.primitive_types) {
                        Some(Ok(ptr)) => ptr,
                    ...
                    let arg_type: TypeLive = match self.get_ptr_value(&arg_type_ptr)? {
                        StoredData::TypeStored(t) => t,
